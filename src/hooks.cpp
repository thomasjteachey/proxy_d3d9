...
static const char* hex24(const uint8_t* p) {
    ...
    for (int i = 0; i < 24; i++) std::snprintf(&buf[i * 3], 4, "%02X ", p[i]);
    buf[3 * 24] = '\0';
    return buf;
}
...
// -------------------- lazy combat hook install --------------------
static void InstallCombatHooks();
static std::atomic<bool> gCombatHooksInstalled(false);
static std::atomic<bool> gCombatHooksKickStarted(false);

static bool GetEnvBoolA(const char* name) {
    char buf[16] = {};
...
static DWORD WINAPI CombatHooksKickThread(LPVOID) {
    __try {
        InstallCombatHooks();
        gCombatHooksInstalled.store(true);
        OutputDebugStringA("[ClientFix] Combat hooks installed (lazy)\n");
    }
    __except (EXCEPTION_EXECUTE_HANDLER) {
        OutputDebugStringA("[ClientFix] Combat hooks install thread crashed; leaving disabled\n");
        gCombatHooksInstalled.store(true);
    }
    return 0;
}

static void MaybeKickCombatHooks()
{
    if (gCombatHooksInstalled.load())
        return;

    // Emergency disable (for startup issues): set CLIENTFIX_DISABLE_COMBAT=1
    if (GetEnvBoolA("CLIENTFIX_DISABLE_COMBAT")) {
        gCombatHooksInstalled.store(true);
        OutputDebugStringA("[ClientFix] Combat hooks disabled via CLIENTFIX_DISABLE_COMBAT\n");
        return;
    }
    ...
    // Wait a bit so the client is fully up and rendering.
    if (sFrames < 30)
        return;

    bool expected = false;
    if (!gCombatHooksKickStarted.compare_exchange_strong(expected, true))
        return;

    HANDLE h = CreateThread(nullptr, 0, CombatHooksKickThread, nullptr, 0, nullptr);
    if (h) CloseHandle(h);
}
...
static HRESULT WINAPI hkEndScene(IDirect3DDevice9* dev)
{
    ...
    if (!sOnce) {
        ...
        HitGate_SetRenderThreadId(tid);
        FrameFence_SetRenderThreadId(tid);
        dbgprintf("[ClientFix][HitGate] hkEndScene first call; tid=%u", tid);
    }

    MaybeKickCombatHooks();
...
{
    // Prevent double-install if something calls this again.
    static std::atomic<bool> sOnce(false);
    bool expected = false;
    if (!sOnce.compare_exchange_strong(expected, true))
        return;

    if (gCombatHooksInstalled.exchange(true)) return;
